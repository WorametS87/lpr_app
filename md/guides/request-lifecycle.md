...
Files: [apps/web/src/pages/InferencePage.tsx](apps/web/src/pages/InferencePage.tsx) → [apps/web/src/controllers/useInferenceController.ts](apps/web/src/controllers/useInferenceController.ts)
...
OCR backend (`easyocr`, `paddle`, or `hybrid`) reads the text from the plate. It tries multiple strategies in order, stopping as soon as it finds a valid result:
...
- **OCR tokens** — raw OCR text fragments + confidence
- **OCR strategy path** — `plateSource` / `ocrRegionUsed` path selected by model
- **Province source info** — `provinceSource` (`classifier` vs `bottom_ocr`)
- **OCR trace** — per-stage trace (`ocrTrace`) with candidates, picks, and optional step image
...
Files: [apps/web/src/components/features/ResultDialog.tsx](apps/web/src/components/features/ResultDialog.tsx) → [apps/web/src/components/features/DebugPanel.tsx](apps/web/src/components/features/DebugPanel.tsx)
...
LPR_OCR_CANVAS_SIZE=1024    # EasyOCR/hybrid canvas size
LPR_OCR_BACKEND=easyocr     # easyocr | paddle | hybrid
LPR_WARMUP_ON_START=1       # warm detector/classifier/OCR on startup
...
│           └── components/features/DebugPanel.tsx
└── packages/
    └── shared-types/
...
| Python model server | Python | ML libraries (PyTorch + OCR backends) only work in Python |
...
OCR is usually the slowest part. On CPU it can take a few seconds, and retry paths (rotation/fallback/repair) can add more time. GPU is faster, but this setup currently recommends CPU for stability.
